...
#if (length(gset) > 1) idx <- grep("GPL2896", attr(gset, "names")) else idx <- 1
gset <- gset[[idx]]
...
gs <- factor(sml)
groups <- make.names(c("acute","control","latent","chronic"))
levels(gs) <- groups
gset$group <- gs
...
volcanoplot(fit2, coef = ct, 
            main = paste("Volcano Plot: ", colnames(fit2)[ct]),  # Title with contrast name
            pch = 20, 
            highlight = length(which(dT[, ct] != 0)), 
            names = rep('+', nrow(fit2)))
# MD plot (log fold change vs mean log expression)
# highlight statistically significant (p-adj < 0.05) probes
plotMD(fit2, column=ct, status=dT[,ct], legend=F, pch=20, cex=1, main=paste(GSE_id, colnames(fit2)[ct]))
abline(h=0)
